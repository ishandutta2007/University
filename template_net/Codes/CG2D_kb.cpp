...
int main()
{
 ...
 //        for(int i = 0; i < 3; i++)
 //        {
 //            theta[i] = pi - v[i].ang(v[(i-1+3)%3]);
 //        }
 //        for(int i = 1; i < 4; i++)
 //        {
 //            int pos = i % 3;
 //            l1 = Line(p[pos], v[pos].ratate(Point(0,0), theta[pos] / 3.0));
 //            l2 = Line(p[(pos+1)%3], rotv(v[(pos+1)%3], theta[(pos+1)%3] / 1.5));
 //            Point tmp = inter_p(l1, l2);
