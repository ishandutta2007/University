...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            theta[i] = PI - Angle(v[i], v[(i-1+3)%3]);
        }
        for(int i = 1; i < 4; i++)
        {
            int pos = i % 3;
            l1 = Line(p[pos], Rotate(v[pos], theta[pos] / 3.0));
            l2 = Line(p[(pos+1)%3], Rotate(v[(pos+1)%3], theta[(pos+1)%3] / 1.5));
            Point tmp = GetlineIntersection(l1, l2);
            printf("%.6lf %.6lf%c", tmp.x, tmp.y, i == 3? '\n' : ' ');
        }
