...
int main()
{
	...
	while(cin >> A >> B >> C >> D >> E)
	{
        ...
        Point a = Point(0.0,0.0), b = Point(1.0,0.0);
        Line bc = Line(b, Rotate(b, (180.0 - D - E)/180.0));
        Line bd = Line(b, Rotate(b, (180.0 - E)/180.0));
        Line ae = Line(a, Rotate(b, C/180.0));
        Line ac = Line(a, Rotate(b, (C+B)/180.0));
        Point e = GetlineIntersection(bc, ae);
        Point d = GetlineIntersection(bd, ac);
        Vec v1 = a-e;
        Vec v2 = d-e;
        double theta = Angle(v1, v2) * 180.0;
