...
//Area2 return sighed area*2 of ABC
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            Vec v = Point(p[(i+1)%3].x + (p[(i + 2)%3].x - p[(i + 1)%3].x) / 3 - p[i].x , p[(i+1)%3].y + (p[(i + 2)%3].y - p[(i + 1)%3].y) / 3 - p[i].y);
            l[i] = Line(p[i], v);
        }
        for(int i = 0; i < 3; i++)
        {
            p[i] = GetlineIntersection(l[i], l[(i+1)%3]);
        }
        printf("%.0lf\n", fabs(Area2(p[0], p[1], p[2])/2));
