  ...
  while(true)
  {
    ...
    for (int l = 1; l <= n; l++)
    {
      ...
      for (int s1 = 0, s2 = l; s2 < n; s1 += l, s2 += l)
      {
        ...
        tmpans = max(tmpans, tmplen/l + 1);
        if (tmplen % l)
        {
          int tmp = l - tmplen % l;
          if (s1 - tmp >= 0)
            tmpans = max(tmpans, callen(s1-tmp, s2-tmp)/l + 1);
        }
      }
      if (tmpans > ans)
      {
    ...
    for (int i = 1; (i <= n) && flag == 0; i++)
      ...
      {
        if (callen(sa[i], sa[i]+anslen[j]) >= (ans-1)*anslen[j])
        {
          bf[sa[i] + anslen[j]*ans] = '\0';
          printf("%s\n", bf + sa[i]);
          flag = 1;
        }
      }
    ...
    //   start = sa[i];
    //   printf("start = %d\n", start);
    //   if (query(i+1, Rank[start+l]) >= anslen - l)
    //   {
