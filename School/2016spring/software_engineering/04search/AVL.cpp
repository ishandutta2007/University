...
public:
    struct Node
    {
      ...
      Node(){}
      Node(int d):data(d),height(1),lson(NULL),rson(NULL){}
    };
    Node *root;
    AVL() { root = NULL; }
    void show(Node *now)
    ...
    bool search(Node *now, int d) {return now == NULL? false:(now->data == d? true:(now->data > d? search(now->lson,d):search(now->rson,d)));}
